        ...
        content += `
            <tr>
                ...
                <td>
                    ...
                    <button data-toggle="modal" data-target="#exampleModal" class="btn btn-info" onclick="getDetailMain('${id}')" >Xem</button>
                </td>
            </tr>
         `
    });
    ...
    foodSer.getFoodDetail(id)
        .then((result) => {
            ...
            for (const key in result.data) {
                // B2: dom tới từng thẻ của form bằng id => key (tên thuộc)
...
let updateFoodMain = () => {
    ...
    let {id, tenMon,loai,gia,phanTram,tinhTrang,hinhAnh,moTa} = foodValue;
    let food = new Food(tenMon, loai, Number(gia), Number(phanTram), tinhTrang, hinhAnh, moTa);
    food.tinhKM();
    console.log(food);
